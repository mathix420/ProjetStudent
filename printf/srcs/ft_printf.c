...
/*   Updated: 2018/12/13 22:14:53 by agissing         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_dioux(t_infos *i, va_list vl, int c, int d)
{
	...
	if (MF_HH & i->data && M_INT & i->data)
		...
		c += ft_putunb((uint8_t)va_arg(vl, uint64_t), i, d);
	else if (MF_H & i->data && M_OUX & i->data)
		c += ft_putunb((uint16_t)va_arg(vl, uint64_t), i, d);
	else if (MF_L & i->data && M_OUX & i->data)
		c += ft_putunb(va_arg(vl, unsigned long), i, d);
	else if (MF_LL & i->data && M_OUX & i->data)
		c += ft_putunb(va_arg(vl, unsigned long long), i, d);
	else if (M_INT & i->data)
		c += ft_putsign(va_arg(vl, int), d, i);
	else if (M_OUX & i->data)
		c += ft_putunb(va_arg(vl, unsigned), i, d);
