...
alias auto_sav="rm -rf ~/goinfre/ProjetStudent__github && gcl https://github.com/mathix420/ProjetStudent.git ~/goinfre/ProjetStudent__github && cd ~/goinfre/ProjetStudent__github/ && yes | git rm -rf * && cp -r ~/42/ProjetStudent/* . && cp ~/.zshrc . && cp ~/.emacs . && find . -type f | grep '/\#\w*\#' | xargs rm && find . -type f | grep '~$' | xargs rm && find ./* -type d | grep .git | xargs rm -rf && git add *  && git add .emacs && git add .zshrc&& git commit -m 'Auto commit $( date )' && git push origin master"
